...
namespace TaskApi.Services.Implementations
{
    public class TaskService : ITaskService
    {
        ...
        public async Task<IEnumerable<TaskDTO>?> Search(string keyword, int page = 1, int size = 10)
        {
            if (page < 1) page = 1;
            if (size < 10) size = 10;
        ...
        public async Task<IEnumerable<TaskDTO>?> Filter(string? keyword, TaskStatus? status, Priority? priority, int page = 1, int size = 10)
        {
            var query = _taskRepository.GetAll();
            ...
            var pagedTask = await query
                .Skip((page - 1) * size)
                .Take(size)
                .ToListAsync();

            return _mapper.Map<IEnumerable<TaskDTO>>(pagedTask);
        }
    }
}